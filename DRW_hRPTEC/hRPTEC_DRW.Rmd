...
Above shows the genes that were used in the activity calculation. The connectivity column is the sum of the TOM rows. A higher connectivity resulted in a higher W_inf, as expected. Term is $W_{∞}$ * log2fc.  
...
```{r}
#saveRDS(GRR_vs_DRW, file = "GRR_vs_DRW_cor_eg.rds") # saving the wGRR for other scoring methods
```
...
ggplot(GRR_vs_DRW, aes(x = drw_zscore)) +
  ...
  geom_density(color = "black") +
  theme_minimal() + labs(title = "Distribution of DRW zScore in RPTEC dataset")
```
