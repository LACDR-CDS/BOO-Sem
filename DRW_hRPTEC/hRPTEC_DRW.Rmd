...
In this Rmd file, the Weighted Directed Random Walk will be performed on the RPTEC dataset.This involves performing a random walk over the network, where nodes present genes and edges present the directions of the network. The procedures described by (Hui et al, 2024) and (Liu et al, 2013) are followed as much as possible. However, they used predefined pathways and this project used co-expressed modules. So, some things will differ and it will be mentioned where it differs. 

First, the directions of the network will be constructed based on the median(TOM) values of genes in a module, if media(TOM(gene A)) > median(TOM(gene B)) than A has an edge to B: A->B (represented as 1). This direction will be weighted by the Topological Overlap Matrix (TOM). *The original article uses predefined pathways for this.* Then, the directions will be reversed, because "a web page is important if other pages point to it" and therefore also genes (Liu et al, 2013). Also,an artificial ground node will be used, which is bidirectionally connected to every gene that has no connections to other genes and one directionally connected to every other gene. This is done to make sure that every node is connected and there are no 0-out of degree connections (Liu et al, 2013), making sure that the progation goes well and the walker converges. This results in the Weighted Graph $G$.
...
$$
Here, the $r$ represents the restart probability, set at 0.7 (Hui et al). This represents the chance to return to the initial weight vector ($W_0$), which represents the initial differential signal for each gene, based on biological and statistical significance: -log10(padj) * abs(Log2FC). *The original article used t-values, but the degrees of freedom are not known. So this score is proposed.*

$$
W_0(g_i) = -\log_{10} \left( p_{\mathrm{adj}}(g_i) \right) \cdot \left| \log_2 \mathrm{FC}(g_i) \right|
$$

The $A_{Mj}$ represents the row normalized adjacency matrix of the Graph $G$. The $W_t$ vector represents the weight of the nodes (genes) at time $t$ and the $W_{t+1}$ vector represents the weight of the nodes (genes) at time $t+1$.
...
Afterwards, the module activty for module $M_j$ will be calculated using the following formula, only using genes that have a padj < 0.05 and an absolute log2fc greater than 1.0:

$$
wDRW(M_j) = \frac{ \sum_{i=1}^{n_j} W_\infty(g_i) \cdot \log_2 \mathrm{FC}(g_i) }
{ \sqrt{n_j} \cdot \sqrt{ \sum_{i=1}^{n_j} \left( W_\infty(g_i) \right)^2 } }
$$

Here, the $W_{∞}$ represents the weight of each gene in the network and log2fc represents the (signed) expression value. The sum of all the $W_{∞}$ vector multiplied by the abs log2fc of the genes will be divided by the square root of the sum of the squared $W_{∞}$ vector for each gene. The result is a  wDRW score that represents the activity value of a modelling, integrating both differential expression and network importance of genes.
...
The TOM will be calculated to later weight the directions of the graphs. 
...
```{r}
TOM   <- readRDS(file.path(data_dir, "TOM_RPTEC.rds"))
```
...
As shown above, the TOM matrix calculation worked and the genes now have different values. Potentially, more topologically correct. This TOM will be used to weight the directions of the graph and to construct directions.

```{r}
#saveRDS(TOM, file = "TOM_RPTEC.rds")
```
...
The initial artical uses t-scores (Hui et al, 2024) for constructing the initial weight (W_0) of the walker, however the degrees of freedom are not known. So therefore, a different score will be used. For this the "walker score" will be used, which is proposed as the abs(Log2FC) * -log10(padj). To account for statistical significance and biological significance. 
...
To construct the Directed Graph for use in the Directed Random Walk. The median (TOM) scores will be used for genes in the same module. If median(TOM(gene A)) > median(TOM(gene B)) than A has an edge to B: A->B. Then the directions will be reversed: a webpage is important if other webpages point to it (Hui et al, 2024). This directed Graph will then be weighted by the TOM. Resulting in a weighted Directed Graph.

## Correlation EGs

No correlation EGs are given for the RPTEC dataset, so these will be calculated first. **this is not used anymore, but was before, so the chunks are still included**

```{r}
expression_cor_eg <- expression_joined %>%
  # Remove module 0, because this is removed in the GRR_vs_eg_score df
  filter(module_number != 0) %>%
  
  # Add eg_score per sample_id + module_number in expression df
  left_join(GRR_vs_eg_score, by = c("sample_id", "module_number")) %>%
  
  # calculate cor_eg per sample_id and module for every gene
  group_by(entrez_id, module_number) %>%
  mutate(
    cor_eg = abs(cor(log2fc, eg_score, method = "pearson", use = "pairwise.complete.obs"))
  ) %>%
  ungroup() %>%
  
  # Keep only relevant columns
  select(entrez_id, sample_id, module_number, log2fc, eg_score, cor_eg)
```
...
## Adding all relevant columns to expression_cor_eg for the walker loop

```{r}
expression_cor_eg <- expression_cor_eg %>%
  left_join(
    expression_joined %>%
      select(entrez_id, sample_id, module_number, padj, walker_score),
    by = c("entrez_id", "sample_id", "module_number")
  )
```

## Calculating the Directed Graph

The directed graph will be calculated in the walker loop for each module+condition.

# Perform the walker on one module to learn how it works practically

Before performing the walker on the whole RPTEC dataset, it will be tested on one module to assess how it works exactly. Module 100 was selected with condition LU_HRPTECTERT1_SINGLE_CITRININ_T3_C3 (random), because this module is small (5 genes) and therefore easy to visualize the weights of the individual genes
...
Again, if median(TOM(A)) > median(TOM(B)) then edge: A -> B. However, these edges will be reverted: a gene is important if other genes point to it (Liu et al, 2013)

```{r}
TOM_medians <- apply(TOM_sub, 1, median, na.rm = TRUE)
```
...
```{r}
# Directional Graph based on TOM median
D <- outer(TOM_medians, TOM_medians, FUN = function(x, y) as.numeric(x < y))
rownames(D) <- colnames(D) <- names(TOM_medians)
...
The weighted directed matrix is correctly normalized. and directions are reversed.
...
```{r}
# Prepare the initial walker probability (W0) based on the proposed walker score. 
  ...
  W0_sig <- expr_sub %>%
    filter(entrez_id %in% rownames(A)) %>% # , padj < 0.05 was left out to reduce over estimation
...
a higher walker score recieves a higher W0.
...
print(W_inf)
```
Above shows the final weights of the walker.
...
As shown above, the L1-norm decreases after iterations, making the recursive walker converge to the base case (L1-norm 10^-10). 
...
Above could be seen how the weight vectors change. It is shown that the W_t vector converges to a stable value, which is below the W0 vector in this case.

## Inferring pathway activity

Calculate the activity of the module with the output $W_{∞}$ for the genes. Only genes with an adjusted p-value < 0.05 will be used in the calculation. Also, the abs value of the log2fc is used, because otherwise the activity score will get canceled out (became clear when running on a subset).

```{r}
activity_df <- expression_joined %>%
  filter(sample_id == condition_id,
         entrez_id %in% names(W_inf),
         padj < 0.05, abs(log2fc) > 1.0) %>%
...
Above shows the genes that were used in the activity calculation. The connectivity column is the sum of the TOM rows.  
...
compute_drw_score <- function(condition_id, module_nr, expression_df, TOM) {
  ...
  # Create directed edges: gene B -> gene A if Median TOM(A) > median TOM(B) as explained above
  TOM_medians <- apply(TOM_sub, 1, median, na.rm = TRUE)
  ...
  # Weight the directed edges by the TOM
...
module_samples <- expression_joined %>%
  ...
  #slice_tail(n = 2930)  # Keep only the last x combinations, for testing the function
```
...
This is a long calculation for the RPTEC dataset and was performed on ALICE.
...
```{r}
drw_scores          <- readRDS(file.path(data_dir, "drw_scores_RPTEC_r0.7.rds")) # reading the drw_scores
```
...
The DRW scores will be normalized for their module_size and called the drw_zscore (it is not really a z-score but this is for simplification)
...
```{r}
#saveRDS(drw_scores, file = "drw_scores_RPTEC_directed.rds") # saving the drw_scores
```


No NA values

# Evaluate the DRW scores on the RPTEC with the GRR and wGRR metric

The DRW scores will be evaluated on the Gene Response Rate and  Weighted Gene Response Rate (wGRR) metric, which is proposed as an evaluation metric for the topological activation of every unique combination of condition and module. The GRR and wGRR are presented as the following formulas:
...
tom_weights <- get_module_relative_tom(TOM, expression_cor_eg)
```
...
weighted_grr_df <- expression_cor_eg %>%
  ...
  summarise(
    ...
    GRR = sum(is_significant, na.rm = TRUE),
    .groups = "drop"
  )
```
...
GRR_vs_DRW <- GRR_vs_DRW %>%
  ...
  mutate(wGRR = wGRR, GRR = GRR / module_size)  #/ module_size
```

```{r}
sum(is.na(GRR_vs_DRW$wGRR))
sum(is.na(GRR_vs_DRW$GRR))
```
...
```{r}
#saveRDS(GRR_vs_DRW, file = "GRR_vs_DRW_cor_eg.rds") # saving the wGRR for other RPTEC scoring methods
```

The wGRR has a reach of 0 to 1 and no NA values, as intended

## Density plots of GRR, wGRR and DRW score

```{r}
ggplot(GRR_vs_DRW, aes(x = abs(GRR))) +
...
```{r}
ggplot(GRR_vs_DRW, aes(x = abs(wGRR))) +
...
Both abs(wGRR) and abs(GRR) log distributed

```{r}
ggplot(GRR_vs_DRW, aes(x = drw_zscore)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "red", alpha = 0.6) +
  geom_density(color = "black") +
  theme_minimal() + labs(title = "Distribution of wDRW score in RPTEC dataset")
```
Normal distribution

# Fitting wDRW vs GRR and wGRR using binomial GLMS

## Visualize GRR and wGRR vs DRW score with a linear fit

```{r}
ggplot(GRR_vs_DRW, aes(x = drw_zscore, y = abs(GRR))) +
...
```{r}
ggplot(GRR_vs_DRW, aes(x = drw_zscore, y = abs(wGRR))) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  theme_minimal() +
  labs(
...
```{r}
glm_data <- GRR_vs_DRW %>%
  filter(!is.na(GRR), !is.na(drw_zscore)) # This filtering was performed as the function was first tested on a subset, but the filtering is irrelevant for the whole dataset as there are no NAs
```


```{r}
glm_fit <- glm(
  abs(GRR) ~ abs(drw_zscore), # absolute because of binomial
  data = glm_data,
...
```{r}
ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(GRR))) +
...
```
There are 4914 outliers found. This is less than the eg score.
...
Above shows the outlier density. n outliers / (n conditions * n modules)

```{r}
ggplot(GRR_glm_fit_df, aes(x = abs(drw_zscore), y = abs(GRR))) +
  geom_point(aes(color = outlier), alpha = 0.6, size = 0.9) +
  geom_line(aes(y = fitted_GRR), color = "blue", size = 1) +
  scale_color_manual(values = c("black", "red")) +
  theme_minimal() +
  labs(
    title = "abs(GRR) vs abs(DRW) Score with Significant Outliers in RPTEC",
    x = "Absolute Weighted Directed Random Walk Score (wDRW)",
    y = "Absolute Gene Response Rate (GRR)"
  )
```

```{r}
p <- ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(GRR))) +
  geom_point(aes(color = outlier), alpha = 0.6, size = 0.9) +
  geom_line(aes(y = fitted_GRR), color = "blue", size = 1) +
  scale_color_manual(values = c("black", "red")) +
...
```{r}
#ggsave("GRR_wDRW_RPTEC.pdf", plot = p, width = 6, height = 4)
```


This fit shows, visually, less over and under quantification compared to the EG. especially the high EG scores (overquantification) with 0 gene responses are not present. Overall, this scoring shows less randomness than the eg scoring method on the proposed evaluation metric.
...
```{r}
ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(wGRR))) +
  geom_point(alpha = 0.5, size = 0.8) +
...
sum(GRR_glm_fit_df$outlier, na.rm = TRUE)
```
There are 6998 outliers found. This is less than the eg score.
...
Again outlier density above

```{r}
ggplot(GRR_glm_fit_df, aes(x = abs(drw_zscore), y = abs(wGRR))) +
  geom_point(aes(color = outlier), alpha = 0.6, size = 0.9) +
  geom_line(aes(y = fitted_wGRR), color = "blue", size = 1) +
  scale_color_manual(values = c("black", "red")) +
  theme_minimal() +
  labs(
    title = "abs(wGRR) vs abs(DRW) Score with Significant Outliers in RPTEC",
    x = "Absolute Weighted Directed Random Walk Score (wDRW)",
...
```{r}
p <- ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(wGRR))) +
  geom_point(aes(color = outlier), alpha = 0.6, size = 0.9) +
  geom_line(aes(y = fitted_wGRR), color = "blue", size = 1) +
  scale_color_manual(values = c("black", "red")) +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
...
```{r}
#ggsave("wGRR_wDRW_RPTEC.pdf", plot = p, width = 6, height = 4)
```


This fit shows, visually, less over and under quantification compared to the EG. especially the high EG scores (overquantification) with 0 gene responses are not present. However, the fit still shows some underquantification. But overall, this scoring shows less randomness than the eg scoring method in the proposed

## Modules and conditions with most significant outliers'

Based on the binomial GLM fit, we will now find the modules and conditions with the most significant outliers. Total, above and below the fit.
...
outliers_per_module <- significant_outliers_filtered %>%
  dplyr::count(module_number, sort = TRUE)
```

```{r}
outliers_per_sample <- significant_outliers_filtered %>%
  dplyr::count(sample_id, sort = TRUE)
```
...
```
Interestingly, other modules and conditions are now found to be the top outliers than the ones found by the eigengene score analysis.
...
GRR_glm_fit_df %>%
  filter(outlier) %>%
  dplyr::count(module, annotation, sort = TRUE) %>%
  ...
  labs(
    title = "Top 20 modules with TOTAL significant GRR outliers (RPTEC) (wDRW)",
    x = "Module",
...
GRR_glm_fit_df %>%
  ...
  labs(
    title = "Top 20 modules with outliers ABOVE the GLM fit (RPTEC) (wDRW)",
    x = "Module",
...
GRR_glm_fit_df %>%
  ...
  labs(
    title = "Top 20 modules with outliers BELOW the GLM fit (RPTEC) (wDRW)",
    x = "Module",
...
outliers_per_sample %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Top 20 conditions with significant wGRR outliers (RPTEC) (wDRW)") +
  theme(
...
GRR_glm_fit_df %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Top 20 conditions with outliers ABOVE the binomial GLM fit (RPTEC) (DRW)") +
  theme(
    plot.title.position = "plot",
    plot.title = element_text(hjust = 0),
...
GRR_glm_fit_df %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Top 20 conditions with outliers BELOW the binomial GLM fit (RPTEC) (wDRW)") +
  theme(
    plot.title.position = "plot",
    plot.title = element_text(hjust = 0),
    plot.margin = margin(20, 20, 10, 40)
...
The DRW shows less outliers and a more 'robust' fit than the EG score on both the GRR and wGRR metric in RPTEC. It will be further assessed using other evaluations if the wDRW is better on other metrics as well.
