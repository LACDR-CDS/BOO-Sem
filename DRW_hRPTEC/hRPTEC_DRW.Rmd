...
The walker will be performed until the L1-norm ,the difference (delta), between $W_t$ and $W_{t+1}$ converges to $10^{–10}$ and this limit, the $W_{∞}$, will be obtained for each gene. The $W_{∞}$ of the ground node will be evenly distributed over the other genes in the module.

Afterwards, the module activty for module $M_j$ will be calculated with the following formula:

$$
DRW(M_j) = \frac{ \sum_{i=1}^{n_j} W_\infty(g_i) \cdot \log_2 \mathrm{FC}(g_i) }{ \sqrt{ \sum_{i=1}^{n_j} \left( W_\infty(g_i) \right)^2 } }
$$
...
expression_cor_eg <- expression_joined %>%
  ...
  mutate(
    cor_eg = cor(log2fc, eg_score, method = "pearson", use = "pairwise.complete.obs")
  ) %>%
  ungroup() %>%
...
The directed graph will be calculated in the walker loop. As every combination of module + sample_id has different cor_egs
...
```{r}
W0 <- expr_sub %>%
  filter(entrez_id %in% rownames(A)) %>%
...
# The ground node will have a starting weight of 0
W0 <- c(W0, GROUND = 0)

# Make sure that the genes match
...
Above could be seen how the weight vectors change. It is shown that the W_t vector converges to a stable value, which is below the W0 vector.

## Inferring pathway activity

Calculate the activity of the module with the output $W_{∞}$ for the genes. Only genes with an adjusted p-value < 0.05 will be used in the calculation.
...
Above shows the genes that were used in the activity calculation. The connectivity column is the sum of the TOM rows. A higher connectivity resulted in a higher W_inf, as expected. Term is $W_{∞}$ * log2fc. This resulted in a DRW score of -0.4664461.  
...
This function will be called for every combination of module and condition in a loop.
...
compute_drw_score <- function(condition_id, module_nr, expression_df, TOM) {
  ...
  W0 <- W0 / sum(W0)
  W0 <- c(W0, GROUND = 0) # Ground node has no starting probability

  # Ensure gene order matches
  stopifnot(all(names(W0) == rownames(A))) # To ensure not having weird results
...
module_samples <- expression_joined %>%
  ...
  #slice_head(n = 100)  # Keep only the first 100 combinations, for testing the function
```
...
This is a very long calculation (~ 120 minutes on my local CPU (a relatively new Intel i5)), so will not be knitted. I tried to parallize it, but did not work.

```{r}
plan(multisession, workers = parallel::detectCores() - 2) # Max CPU cores -2 will be used with these settings
...
```{r}
saveRDS(drw_scores, file = "drw_scores_RPTEC_directed.rds")
```
...
drw_scores <- drw_scores %>%
  mutate(abs_drw_score = abs(drw_score))
```

```{r}
sum(is.na(drw_scores$abs_drw_score))
```


# Evaluate the DRW scores on the RPTEC with the GRR metric

The DRW scores will be evaluated on the Gene Response Rate (GRR) metric to compare DRW to the other scoring methods. The GRR metric represents the proportion of significant genes in a module, where significant represents log2FC > 1.0 and padj < 0.05. The scripts for this follow the same set up as the eigengene evaluations (hRPTEC_EG)
...
```{r}
ggplot(GRR_vs_DRW, aes(x = drw_score)) +
...
```{r}
ggplot(GRR_vs_DRW, aes(x = abs_drw_score)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "red", alpha = 0.6) +
  geom_density(color = "black") +
  theme_minimal() +
  labs(title = "Distribution of Absolute DRW score in RPTEC dataset")
```

## Visualize GRR vs DRW score with a linear fit

```{r}
ggplot(GRR_vs_DRW, aes(x = abs_drw_score, y = GRR)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  theme_minimal() +
  labs(
    title = "Gene Response Rate vs DRW Score in RPTEC",
    x = "Directed Random Walk score (DRW)",
...
```{r}
glm_fit <- glm(
  cbind(sig_genes, module_size - sig_genes) ~ abs_drw_score,
...
```{r}
ggplot(GRR_glm_fit_df, aes(x = abs_drw_score, y = GRR)) +
  geom_point(alpha = 0.5, size = 0.8) +
  geom_line(aes(y = fitted_sig_genes / module_size), color = "blue", size = 1) +
  theme_minimal() +
  labs(
    title = "Binomial GLM fit: GRR vs DRW score in RPTEC",
    x = "Directed Random Walk score (DRW)",
    y = "Gene Response Rate (GRR)"
  )
```
...
ggplot(GRR_glm_fit_df, aes(x = abs_drw_score, y = GRR)) +
  ...
  theme_minimal() +
  labs(
    title = "GRR vs DRW Score with Significant Outliers in RPTEC",
    x = "Directed Random Walk Score (DRW)",
    y = "Gene Response Rate (GRR)"
  )
```
...
GRR_glm_fit_df %>%
  ...
  labs(
    title = "Top 20 modules with TOTAL significant GRR outliers (RPTEC) (DRW)",
    x = "Module",
...
GRR_glm_fit_df %>%
  ...
  labs(
    title = "Top 20 modules with outliers ABOVE the GLM fit (RPTEC) (DRW)",
    x = "Module",
...
GRR_glm_fit_df %>%
  ...
  labs(
    title = "Top 20 modules with outliers BELOW the GLM fit (RPTEC) (DRW)",
    x = "Module",
...
outliers_per_sample %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Top 20 conditions with significant GRR outliers from the binomial GLM fit (RPTEC) (DRW)") +
  theme(
...
GRR_glm_fit_df %>%
  ...
  coord_flip() +
  theme_minimal() +
  ggtitle("Top 20 conditions with outliers BELOW the binomial GLM fit (RPTEC) (DRW)") +
  theme(
    plot.title.position = "plot",
    plot.title = element_text(hjust = 0),
    plot.margin = margin(20, 20, 10, 40)
