...
weighted_grr_df <- expression_cor_eg %>%
  ...
  summarise(
    wGRR = sum(is_significant * cor_eg, na.rm = TRUE),
    .groups = "drop"
  )
```
...
```{r}
ggplot(GRR_vs_DRW, aes(x = abs(drw_zscore), y = abs(wGRR))) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  theme_minimal() +
  labs(
    title = "Weighted Gene Response Rate (wGRR) vs DRW Score in RPTEC",
