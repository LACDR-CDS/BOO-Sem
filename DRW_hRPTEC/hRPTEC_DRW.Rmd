...
ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(GRR))) +
  ...
  theme_minimal() +
  labs(
    title = "abs(GRR) vs DRW Score with Significant Outliers in RPTEC",
    x = "Weighted Directed Random Walk Score (wDRW)",
    y = "Absolute Gene Response Rate (GRR)"
  )
```
...
sum(GRR_glm_fit_df$outlier, na.rm = TRUE)
```
There are 4677 outliers found. This is less than the eg score.
...
ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(wGRR))) +
  ...
  theme_minimal() +
  labs(
    title = "abs(wGRR) vs DRW Score with Significant Outliers in RPTEC",
    x = "Weighted Directed Random Walk Score (wDRW)",
    y = "Absolute Weighted Gene Response Rate (wGRR)"
  )
```
