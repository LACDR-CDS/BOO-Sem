...
In this Rmd file, the Weighted Directed Random Walk will be performed on the PHH dataset.This involves performing a random walk over the network, where nodes present genes and edges present the directions of the network. The procedures described by (Hui et al, 2024) and (Liu et al, 2013) are followed as much as possible. However, they used predefined pathways and this project used co-expressed modules. So, some things will differ and it will be mentioned where it differs. 
...
head(TOM[, 1:5])
```
The TOM now have correct row and column names.
...
ggplot(expression_long, aes(x = walker_score)) +
  ...
  theme_minimal() + labs(title = "Distribution of walker score (W0) in PHH dataset")
```
...
To construct the Directed Graph for use in the Directed Random Walk. The Median(TOM) scores will be used for genes in the same module. If median(TOM(gene A)) > median(TOM(gene B)) than A has an edge to B: A->B. Then the directions will be reversed: a webpage is important if other webpages point to it (Hui et al, 2024). This directed Graph will then be weighted by the TOM values. Resulting in a weighted Directed Graph.

## Correlation EGs

The correlation EG, calculated by LACDR, will be joined with the expression_joined df. **The cor_egs are not used anymore, but are still included here.**
...
# Perform the walker on one module to learn how it works

Before performing the walker on the whole PHH dataset, it will be tested on one module to assess how it works exactly. Module 295 was selected with condition "TG_HPHH_SINGLE_ACETAMINOPHEN_T3_C3".

## Defining the module and condition for the testing 

```{r}
module_nr = 295
condition_id = "TG_HPHH_SINGLE_ACETAMINOPHEN_T3_C3"
```
...
```
a higher walker score recieves a higher W0 as intended.
...
Above could be seen how the weight vectors change. It is shown that the W_t vector converges to a stable value, which is below the W0 vector in this module.

## Inferring pathway activity

Calculate the activity of the module with the output $W_{∞}$ for the genes. Only genes with an adjusted p-value < 0.05 and log2fc > 1.0 will be used in the calculation. 
...
```{r}
drw_scores          <- readRDS(file.path(data_dir, "drw_scores_PHH_r0.7.rds")) # reading the drw_scores
```
...
#drw_scores <- drw_scores %>%
 # left_join(
  ...
  #) %>%
  #mutate(
   # drw_zscore = drw_score / sqrt(module_size)
  #)
```
...
```{r}
#saveRDS(drw_scores, file = "drw_scores_PHH_r0.7.rds") # saving the drw_scores
```
...
The wDRW scores will be evaluated on the Gene Response Rate and  Weighted Gene Response Rate (wGRR) metric, which is proposed as an evaluation metric for the topological activation of every unique combination of condition and module. The GRR and wGRR are presented as the following formulas:
...
GRR_vs_DRW <- drw_scores %>%
  left_join(
    GRR_vs_eg_score %>% select(sample_id, module_number, GRR, wGRR),
    by = c("condition_id" = "sample_id", "module_nr" = "module_number")
  )
```
...
ggplot(GRR_vs_DRW, aes(x = abs(GRR))) +
  ...
  theme_minimal() + labs(title = "Distribution of Gene Response Rate (GRR) in PHH dataset")
```
...
ggplot(GRR_vs_DRW, aes(x = abs(wGRR))) +
  ...
  geom_density(color = "black") +
  theme_minimal() + labs(title = "Distribution of Weighted Gene Response Rate (wGRR) in PHH dataset")
```
...
ggplot(GRR_vs_DRW, aes(x = drw_zscore)) +
  ...
  geom_density(color = "black") +
  theme_minimal() + labs(title = "Distribution of wDRW score in PHH dataset")
```
Normal distribution
...
ggplot(GRR_vs_DRW, aes(x = drw_zscore, y = abs(GRR))) +
  ...
  theme_minimal() +
  labs(
    title = "Abs Gene Response Rate (GRR) vs DRW Score in PHH",
...
ggplot(GRR_vs_DRW, aes(x = drw_zscore, y = abs(wGRR))) +
  ...
  theme_minimal() +
  labs(
    title = "Abs Weighted Gene Response Rate (wGRR) vs DRW Score in PHH",
    x = "Weighted Directed Random Walk score (wDRW)",
...
ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(GRR))) +
  ...
  theme_minimal() +
  labs(
    title = "Binomial GLM fit: Abs GRR vs DRW in PHH",
    x = "Weighted Directed Random Walk Score (wDRW)",
...
```{r}
GRR_glm_fit_df <- GRR_glm_fit_df %>%
  mutate(module = paste0("hPHH_", module_nr)) %>%
...
There are 6625 outliers found. This is less than the eg score outliers for GRR

```{r}
total_modules_all <- GRR_glm_fit_df %>%
  filter(module_nr != 0) %>%
  summarise(n = n_distinct(module_nr)) %>%
...
total_conditions_all <- GRR_glm_fit_df %>%
  filter(module_nr != 0) %>%
  summarise(n = n_distinct(condition_id)) %>%
  pull(n) # for normalization
```
...
Above shows the outlier density. n outliers / (n_conditions * n_modules
...
ggplot(GRR_glm_fit_df, aes(x = abs(drw_zscore), y = abs(GRR))) +
  ...
  theme_minimal() +
  labs(
    title = "abs(GRR) vs abs(DRW) Score with Significant Outliers in PHH",
...
```{r}
#ggsave("GRR_wDRW_PHH.pdf", plot = p, width = 6, height = 4)
```
...
```{r}
glm_data <- GRR_vs_DRW %>%
  filter(!is.na(wGRR), !is.na(drw_zscore), abs(wGRR)<=1) # This filtering was performed as the function was first tested on a subset, but the filtering is irrelevant for the whole dataset as there are no NAs. Also there was an error with wGRR because one value is above 1?
```
...
ggplot(GRR_glm_fit_df, aes(x = drw_zscore, y = abs(wGRR))) +
  ...
  theme_minimal() +
  labs(
    title = "Binomial GLM fit: wGRR vs DRW in PHH",
    x = "Weighted Directed Random Walk Score (wDRW)",
...
There are 6628 outliers found. This is less than the eg score.
...
ggplot(GRR_glm_fit_df, aes(x = abs(drw_zscore), y = abs(wGRR))) +
  ...
  theme_minimal() +
  labs(
    title = "abs(wGRR) vs abs(DRW) Score with Significant Outliers in PHH",
    x = "Absolute Weighted Directed Random Walk Score (wDRW)",
...
```{r}
#ggsave("wGRR_wDRW_PHH.pdf", plot = p, width = 6, height = 4)
```
...
outliers_per_module <- significant_outliers_filtered %>%
  count(module_nr, sort = TRUE)
```

```{r}
outliers_per_sample <- significant_outliers_filtered %>%
  count(condition_id, sort = TRUE)
```
...
```{r}
threshold <- 70  # Threshold for switching annotation position
...
outliers_per_sample %>%
  top_n(20, n) %>%
  ggplot(aes(x = reorder(condition_id, n), y = n)) +
...
The DRW shows less outliers and a more 'robust' fit than the EG score on both the GRR and wGRR metric. It will be further assessed using other evaluations if the wDRW is better on other metrics as well.