...
drw_scores <- drw_scores %>%
  left_join(
    GRR_vs_eg_score %>% select(sample_id, module_number, module_size.x),
    by = c("condition_id" = "sample_id", "module_nr" = "module_number")
  ) %>%
  mutate(
    drw_zscore = drw_score / module_size.x
  )
```
...
ggplot(GRR_vs_DRW, aes(x = drw_zscore, y = wGRR)) +
  ...
  theme_minimal() +
  labs(
    title = "Gene Response Rate vs DRW Score in PHH",
...
GRR_glm_fit_df <- GRR_vs_DRW %>%
  mutate(
    ...
    residual = sig_genes.x - fitted_sig_genes,
