...
In this .Rmd file, the top-k scoring modules are identified for different scoring algorithms and conditions. For each compound, the Reactome-based enrichment of these modules is compared to the compound enrichment from the Comparative Toxicogenomics Database(CTD) database. Only Reactome pathways were used for this comparison, as it was the only source shared between the module enrichment results and the CTD data. This analysis serves to evaluate the biological relevance of the proposed module scoring methods. It is assessed if there are matches between the databases (0= no match, 1 = 1 or more match). Then the accuracy for the top-k pathways per scoring method and dataset will be computed to evaluate if DRW based scoring provide more biologically accurate results
...
```{r}
top50_results_PHH_eg <- eg_scores_PHH %>%
  arrange(desc(abs_eg_score)) %>%
  slice_head(n = 50) %>%
...
```{r}
top50_results_PHH_drw <- drw_scores_PHH %>%
  arrange(desc(drw_zscore)) %>%
  slice_head(n = 50) %>%
  transmute(chemical_ctd,
            module_nr = module_number,
...
eg_scores_RPTEC <- eg_scores_RPTEC %>%
  mutate(chemical_ctd = case_when(
    ...
    chemical == "MITOMYCINC" ~ "mitomycin C-DNA adduct",
...
```{r}
top50_results_RPTEC_eg <- eg_scores_RPTEC %>%
  arrange(desc(abs_eg_score)) %>%
  slice_head(n = 50) %>%
  transmute(chemical_ctd,
            module_nr = module_number,
...
```{r}
top50_results_RPTEC_drw <- drw_scores_RPTEC %>%
  arrange(desc(drw_zscore)) %>%
  slice_head(n = 50) %>%
  transmute(chemical_ctd,
            module_nr = module_number,
...
k_values <- c(10, 20, 30, 40, 50) # for now k is in 10, 20, 30, 40, 50
```
...
# combined results
result_dfs <- list(
  RPTEC_drw = top50_results_RPTEC_drw,
  RPTEC_eg  = top50_results_RPTEC_eg,
  PHH_drw   = top50_results_PHH_drw,
  PHH_eg    = top50_results_PHH_eg
)
```
...
We want to compare across k. so pivot wide

```{r}
accuracy_wide <- accuracy_results %>%
  select(dataset, k, accuracy) %>%
...
```{r}
ggplot(accuracy_results, aes(x = dataset, y = accuracy, fill = factor(k))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Accuracy of Top-k Scoring Pathway per Scoring Method",
    x = "Scoring Algorithms and Dataset",
    y = "Accuracy",
    fill = "Top-k Pathways"
  ) +
