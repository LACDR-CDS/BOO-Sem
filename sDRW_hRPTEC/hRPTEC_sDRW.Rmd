...
In this .Rmd file, the Weighted Significant Directed Random Walk (s-wDRW) will be evaluated. This scoring algorithm tweaks the 'basic'Directed Random Walk a little and therefore the same procedures will be followed. It tweaks the walker formula, using this expression:

$$
W_{t+1} = (1 - r) A \left( \frac{N_1 + N_2}{2} \right) + r W_t
$$

Here $A$ represents the row-normalized adjacency matrix, described below. $r$ represents the restart probability, set at 0.7. $N1$ and $N2$ present the 

## Introduction 'basic' Directed Random Walk

**The 'basic' DRW introduction is given for comparison, this procedure will be largely followed, except for the changes given above**

In this Rmd file, the 'basic' Directed Random Walk will be performed on the RPTEC dataset.This involves performing a random walk over the network, where nodes present genes and edges present the directions of the network. The procedures described by (Hui et al, 2024) and (Liu et al, 2013) are followed as much as possible. However, they used predefined (KEGG) pathways and this project used co-expressed modules. So, some things will differ and it will be mentioned where it differs. 

First, the directions of the network will be constructed based on the correlation eigengenes  in a module, if cor_eg(gene A) > cor_eg(gene B) than A has an edge to B: A->B (represented as 1). This direction will be weighted by the Topological Overlap Matrix (TOM). *The original article uses predefined pathways for this.* The TOM accounts for indirect gene correlations. Then, the directions will be reversed, because "a web page is important if other pages point to it" and therefore also genes (Liu et al, 2013). Also,an artificial ground node will be used, which is bidirectionally connected to every gene. This is done to make sure that every node is connected and there are no 0-out of degree connections (Liu et al, 2013)

After that, the random walker will be performed: 

$$
W_{t+1} = (1 - r) A W_t + r W_0
$$
Here, the $r$ represents the restart probability, set at 0.7 (Hui et al). This represents the chance to return to the initial weight vector ($W_0$), which represents the "walker score" for each gene: -log10(padj) * abs(Log2FC). *The original article used t-values, but the degrees of freedom are not known. So the walker score is proposed.* 

$$
W_0(g_i) = -\log_{10} \left( p_{\mathrm{adj}}(g_i) \right) \cdot \left| \log_2 \mathrm{FC}(g_i) \right|
$$

The $A$ represents the row normalized directed TOM. The $W_t$ vector represents the weight of the nodes (genes) at time $t$ and the $W_{t+1}$ vector represents the weight of the nodes (genes) at time $t+1$.
...
$$
wDRW(M_j) = \frac{ \sum_{i=1}^{n_j} W_\infty(g_i) \cdot |\log_2 \mathrm{FC}(g_i)| }{ \sqrt{ \sum_{i=1}^{n_j} \left( W_\infty(g_i) \right)^2 } }
$$
Here, the $W_{∞}$ represents the weight of each gene in the network and log2fc represents the expression value. The sum of all the $W_{∞}$ vector multiplied by the log2fc of the genes will be divided by the square root of the sum of the squared $W_{∞}$ vector for each gene. Only genes with a padj value < 0.05 will be used to construct the module activity.
...
No correlation EGs are given for the RPTEC dataset, so these will be calculated first.
...
```{r}
# Run random walk with restart using sDRW update rule
r <- 0.7
W_prev <- W0
...
for (i in 1:1000) {
  W_avg <- numeric(length(W_prev))
  names(W_avg) <- names(W_prev)

  for (j in seq_along(W_prev)) {
    incoming <- which(A[, j] > 0)
    if (length(incoming) == 0) next

    weights <- A[incoming, j]
    avg_vals <- (W_prev[incoming] + W_prev[j]) / 2
    W_avg[j] <- sum(weights * avg_vals)
  }

  # Normalize to prevent weight explosion
  W_avg_sum <- sum(W_avg)
  if (W_avg_sum == 0 || is.na(W_avg_sum)) {
    return(tibble(condition_id = condition_id, module_nr = module_nr, drw_score = 0))
  }
  W_avg <- W_avg / W_avg_sum

  W_new <- (1 - r) * W_avg + r * W_prev
...
ggplot(convergence[-1, ], aes(x = iteration, y = delta)) +
  ...
  scale_y_log10() +
  labs(title = paste("Convergence of sDRW for Module", module_nr),
...
```{r}
# Output
cat("sDRW pathway activity score for module", module_nr, "in condition", condition_id, ":\n")
...
Above shows the genes that were used in the activity calculation. The connectivity column is the sum of the TOM rows. A higher connectivity resulted in a higher W_inf, as expected. Term is $W_{∞}$ * abs log2fc.  
...
```{r}
compute_sdrw_score <- function(condition_id, module_nr, expression_df, TOM) {
  ...
  if (sum(W0) == 0 || any(is.na(W0))) {
    return(tibble(condition_id = condition_id, module_nr = module_nr, sdrw_score = 0))
  }
...
names(W_inf) <- rownames(A)
  ...
  tibble(condition_id = condition_id, module_nr = module_nr, sdrw_score = pathway_activity_score)
}
```
...
```{r}
drw_test_result <- compute_sdrw_score(
...
module_samples <- expression_joined %>%
  ...
  #slice_tail(n = 29)  # Keep only the last x combinations, for testing the function
```
...
This is a long calculation for the RPTEC dataset (~ 120 minutes on my local CPU (a relatively new Intel i5)), so will not be knitted. I tried to parallize it, but did not work. Probably due to not having enough RAM (8 GB) relative to the size of the data.
...
```
```{r}
#saveRDS(sdrw_scores, file = "drw_scores_RPTEC_directed.rds") # saving the drw_scores
```


```{r}
sdrw_scores          <- readRDS(file.path(data_dir, "sdrw_scores_RPTEC_directed.rds")) # reading the drw_scores
```


```{r}
print(head(sdrw_scores))
print(tail(sdrw_scores))
```


```{r}
sum(is.na(sdrw_scores$sdrw_score))
```
```{r}
sdrw_scores$sdrw_score[is.na(sdrw_scores$sdrw_score)] <- 0
```
...
# Evaluate the DRW scores on the RPTEC with the wGRR metric
...
The wGRR has a reach of 0 to 1 (will never reach one practically, due to cor_eg), as expected.
...
```{r}
ggplot(GRR_vs_DRW, aes(x = sdrw_zscore)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "red", alpha = 0.6) +
  geom_density(color = "black") +
  theme_minimal() + labs(title = "Distribution of DRW score in RPTEC dataset")
```
DRW score also logarithmic distributed due to using the abs(log2fc) in the pathway activity inference

## Visualize GRR vs DRW score with a linear fit

```{r}
ggplot(GRR_vs_DRW, aes(x = sdrw_zscore, y = wGRR)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  theme_minimal() +
  labs(
    title = "Weighted Gene Response Rate (wGRR) vs s-wDRW Score in RPTEC",
    x = "Weighted Significant Directed Random Walk score (s-wDRW)",
...
```{r}
glm_data <- GRR_vs_DRW %>%
  filter(!is.na(wGRR), !is.na(sdrw_zscore)) # This filtering was performed as the function was first tested on a subset, but the filtering is irrelevant for the whole dataset as there are no NAs
```
...
```{r}
ggplot(GRR_glm_fit_df, aes(x = sdrw_zscore, y = wGRR)) +
  geom_point(alpha = 0.5, size = 0.8) +
  geom_line(aes(y = fitted_sig_genes / module_size), color = "blue", size = 1) +
  theme_minimal() +
  labs(
    title = "Binomial GLM fit: wGRR vs s-wDRW in RPTEC",
    x = "Weighted Significant Directed Random Walk Score (s-wDRW)",
    y = "Weighted Gene Response Rate (wGRR)"
  )
```
...
```
There are 7134 outliers found. This is less than the eg score.
...
ggplot(GRR_glm_fit_df, aes(x = sdrw_zscore, y = wGRR)) +
  ...
  theme_minimal() +
  labs(
    title = "wGRR vs s-wDRW Score with Significant Outliers in RPTEC",
    x = "Weighted Significant Directed Random Walk Score (s-wDRW)",
    y = "Weighted Gene Response Rate (wGRR)"
  )
```
...
The wDRW shows less outliers and a more 'robust' fit than the EG score on the wGRR metric. This will be further improved by other DRW variants that weight the topological importance even more. (s-DRW and e-DRW)
