...
A_{M_j} \;=\;
\mathrm{if}\,\bigl[\,
\mathrm{median}\bigl(\mathrm{TOM}(A)\bigr)
>
\mathrm{median}\bigl(\mathrm{TOM}(B)\bigr)
\,\bigr]
\;\mathrm{then}\;
A \leftarrow B \cdot \mathrm{TOM}
$$
...
Eigengene shows a normal distribution, as expected due to the PCA use. Abs GRR and wGRR show a log distribution, as expected due to the abs transformation.
...
A lot of points with high scores and low gene response
...
```
There are 10464 outliers found from the binomial fit.
...
ggplot(GRR_glm_fit_df, aes(x = eg_score, y = abs(GRR))) +
  ...
  theme_minimal() +
  labs(
    title = "abs(GRR) vs EG Score with Significant Outliers in PHH",
    x = "Eigengene Score (EG)",
    y = "Absolute Gene Response Rate (GRR)"
  )
```
...
```{r}
glm_fit <- glm(
  abs(wGRR) ~ abs(eg_score), # Absolute bec
  data = glm_data,
  family = quasibinomial(link = "logit") # quasibinomial as the wGRR is a proportion multiplied by a continuous variable, the cor_eg.
)
```
...
sum(GRR_glm_fit_df$outlier, na.rm = TRUE)
```
There are 10654 outliers found.
...
ggplot(GRR_glm_fit_df, aes(x = eg_score, y = abs(wGRR))) +
  ...
  theme_minimal() +
  labs(
    title = "abs(wGRR) vs EG Score with Significant Outliers in PHH",
    x = "Eigengene Score (EG)",
    y = "Absolute Weighted Gene Response Rate (wGRR)"
  )
```
